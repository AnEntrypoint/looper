...
- **signalsmith-stretch configuration**: Configured with blockSamples=512, intervalSamples=192, yielding ~5.3ms output latency at 48kHz (blockSamples/2 ÷ sampleRate). Hop size reduced from 256→192 samples for 25% more overlaps per second and better phase continuity, improving pitch shift quality without latency increase. Wrapper buffers input in 512-sample temp arrays and calls `m_stretch.process()` on each feedAudio/retrieveAudio call pair.
