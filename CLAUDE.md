...
- `CUSBAudioDevice::Configure()` seeds `StartInRequest()` but NOT `StartOutRequest()`. `OutCompletion` re-submits on each callback but is never triggered without the initial call — USB audio output is permanently silent. Fix: add `if (m_pEndpointOut) StartOutRequest();` in `Configure()` after the `StartInRequest` call (`patches/usbaudiodevice.cpp`).
