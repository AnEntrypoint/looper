...
- **WLAN + Link is OPT-IN, compile-gated on `LOOPER_ENABLE_WLAN` (default OFF).** This is the FIRST thing to check when "it didn't connect to ticker / no Link": the default build compiles out `m_WLAN.Initialize()`, `JoinOpenNet`, the AP fallback, and `linkInit()` entirely (`#ifdef LOOPER_ENABLE_WLAN` in `patches/kernel.cpp`), so there is no radio bring-up at all. The `:4445 WLAN` verb reports `wlan=disabled` in that build. Build `LOOPER_ENABLE_WLAN=1` to bring it up. **The two historical wedge mechanisms that forced it off are now mitigated in code** (the old kernel.cpp/Makefile "p9 stack overflow" / "SendFrame wedge" comments were stale): (1) the ~90s assert-halt was the p9 error-stack down-counter leak — `patches/p9error.cpp` self-heals (clamp/reseed, never asserts) and no longer does blocking syslog UDP on the `error()` longjmp hot path (the AGENTS no-blocking-IO rule); witnessed by `scripts/test-p9-errorstack.cpp` (ALL PASS: balanced nets to baseline, leak/overflow storms never wrap or index OOB). (2) the "Core 2 control plane dead within a tick" SendFrame wedge is gated — `abletonLink.cpp::linkProcess()` early-returns unless `CBcm4343Device::IsLinkUp()` (non-blocking cached assoc read), plus a bounded 64-frame/tick RX drain. **Remaining gate = HARDWARE re-validation** (radio stability under sustained audio load is not host-provable); flip the `Makefile` default only after a Pi run confirms a stable control plane. Live error rate is observable without syslog via the `:4445 WLAN` verb `p9err=<count>` (free-running counter from `p9ErrorCount()`, replaced the per-error syslog write).
...
- **FULL Ableton Link (discovery + measurement + ghost timeline), bit-exact for Live interop.** The protocol is implemented as three pure, host-tested header modules shared by firmware + tests, wired into `abletonLink.cpp`:
  - **`linkWire.h`** — bit-exact wire codec. Discovery `_asdp_v\x01` multicast `224.76.78.75:20808`: MessageHeader {u8 type; u8 ttl; u16 groupId; u8 nodeId[8]} BE, types ALIVE=1/RESPONSE=2/BYEBYE=3. Measurement `_link_v\x01` unicast, PING=1/PONG=2. Payload entry = key(u32 BE)+size(u32 BE)+value; all ints BE; micros=i64 BE. TLV keys `tmln`/`sess`/`mep4` + `__ht`/`__gt`/`_pgt`. `tmln` = microsPerBeat(i64) + beatOrigin(**microbeats**, i64) + timeOrigin(i64); `mep4` = IPv4(4 octets) + port(2, BE). CONFIRMED against the reference, a real-Live Wireshark dissector (offsets 28/36/44/52/60), and the Rust reimpl. The OLD impl used the WRONG key `mmbe`+nodeId — Live never measured us. (`scripts/test-link-wire.cpp`.)
  - **`linkGhost.h`** — `GhostXForm{slope=1, offset=median(samples)}`; host↔ghost map (`ghost=host+offset`); the measurement accumulator (sample collect, median, done@100/exhausted@5-pings); and the ghost timeline math (`microbeats(ghost)=beatOrigin+(ghost-timeOrigin)*1e6/mpb`, beat phase mod quantum). Sample math per Live: `s1=ghost-(host+prevHost)/2`, `s2=(ghost+prevGhost)/2-prevHost`. (`scripts/test-link-ghost.cpp`, `scripts/test-link-session.cpp`.)
  - **`linkSession.h`** — peer table {nodeId,MAC,ipv4,mep4Port,timeline,meas,xform,lastSeen}; **AP mode learns each peer's MAC from the Ethernet SOURCE of its frames, so NO ARP**; ownership = highest NodeId (peers with a known timeline); per-peer measurement isolation; expiry (5s) + ownership hand-back; capacity clamp (8); self-NodeId ignore. (`scripts/test-link-session.cpp`.)
- **`abletonLink.cpp` ties it together (runs on Core 2 in `linkProcess`):** sends ALIVE (mep4=our IP:20808, sess, tmln) every 1s + RESPONSE to inbound ALIVE + BYEBYE on `linkShutdown`; the single RX demux routes multicast `_asdp_v` discovery AND unicast `_link_v` measurement (to our IP:mep4 port) — `wlanDhcpServeFrame`/`wlanDhcpClientFrame` still get :67/:68; the **PING responder** replies PONG (sess+ghost+prevGhost+echoed host) to the frame source (reactive, post-RX so radio is live); the **measurement initiator** pings each endpoint-known peer (5 @ 50ms), accumulates, sets the peer GhostXForm; owner election adopts the owner's timeline (our ghost xform = the measured offset to it) and republishes the shared beat phase. Self-clock = `CTimer::GetClockTicks()` (microseconds; same clock the 1Hz beacon uses). Proactive pings/beacons gated on `IsLinkUp()`.
- **masterPhase phase-sync (full sync, done safely).** Core 2 publishes the session beat phase via `linkGhostPhase()` → `apcKey25::update` → `LiveParams.linkBeatPhaseMicroBeats/linkQuantumMicroBeats/linkPhaseValid` (paramSnapshot, torn-safe). `loopMachine::update` (Core 1), when synced + phase-valid + **idle (no running clips)**, tracks the session sub-beat phase into `m_masterPhase` (blocks-per-beat from Live's tempo) so the FIRST recorded loop's downbeat lands on Live's beat — the whole looper then inherits Live's phase through that first loop. It NEVER mutates `m_masterPhase` while clips run (the consecutive/pause/resume phase logic owns it) and does NOT pre-set `m_masterLoopBlocks` (the first loop still defines the phrase; the synced+anyRecorded branch time-stretches it to Live's tempo). Continuous running-loop drift-nudge is deferred to hardware validation.
- **`:4445 LINK` verb** (capture-free, OUR view): `synced peers owner=self/peer offsetUs pingsTx pongsRx phaseValid beatPhase100 bpm`.
- **Link-driven quantize**: when synced, `m_masterLoopBlocks = round(690 * 60 * 16 / bpm + 0.5)` rounded to multiple of 8 (4-bar phrase).
- **HARDWARE VALIDATION PENDING (`link-hw-validate`):** the protocol stack + integration are fully host-tested, but real Live interop (the bit-exact wire format on the air, the measurement convergence, the phase lock by ear) can only be confirmed on the Pi against a real Link app. Flash `dist/kernel7l-wlan.img`, join "ticker" from a laptop running Live, watch `:4445 LINK` (`peers>=1`, `offsetUs` stable, `phaseValid=1`).
