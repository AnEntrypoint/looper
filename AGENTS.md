...
- **Formant depth knob** (CC53): single-knob, ∈ [-1, +1] centred at data2=64 with ±4 deadzone.
  ...
  - **Implemented as a zero-latency LPC envelope-remap POST stage** (`patches/lpcFormant.h`, applied to the `EngineSoladSnac` −12 output). The earlier pre-resample approach is REMOVED — it provably could not shift formants (resampling moves pitch+formant together and cancels against the −12 stage; measured spectral centroid did not move across the knob, it only added doubling/crackle/gurgle). The LPC stage: causal autocorrelation LPC (order 12) on a trailing past-only window per 128-sample hop → read-only Durand-Kerner root-find for the formant pole frequencies → a peaking-biquad bank that CUTS at each formant freq `f` and BOOSTS at `f·shift`, sliding the spectral envelope. `shift = pow(1.5, depth)`. Biquad EQ is **unconditionally stable** (rebuilding the all-pole synthesis filter from frequency-warped poles rang to infinity at order 12 — do NOT do that); coefficient ramping (1/48 per sample) kills hop-swap clicks. **The −12 pitch path is byte-identical to the original great one** (main read at exactly `m_scale`, depth=0 is a bit-clean bypass). Witness `scripts/test-formant-lpc.cpp`: pitch exact −12 at every depth, envelope band-ratio rises monotonically with shift. Research basis: Markel & Gray / Columbia E4896 polewarp; HiFi-Glot (arXiv 2409.14823) and differentiable-TVLP (2406.05128) confirm the source-filter core.
