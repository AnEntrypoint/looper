...
- **Playback-advance rule (single predictable rule, `loopClipUpdate.cpp`):** keyed on state then (has-master, L vs M). `CS_RECORDING_TAIL` → monitor wrap, no state change. Sub-phrase (M>0, L<M) → `play_block = (masterPhase − recordStartPhaseOffset) % L` (phase-locked to grid; L divides M so coherent), wrap→LOOPING gated on `CS_PLAYING`. Else (first loop L==M, long L≥M, no-master) → self-advance, wrap→`_startCrossFade` gated on `CS_PLAYING`. The wrap/crossfade transition is ALWAYS `CS_PLAYING`-gated so recording states never trigger it.
- **Start-phase grids:** `_startRecording` sets `m_recStartBlock` (audio content start = backdated press) and `m_recordStartPhaseOffset` (beat-snapped M/16 when M>0, raw when M==0). `_startEndingRecording` re-snaps `recordStartPhaseOffset` to the QUANT grid `= min(L, M)` (division below a phrase, phrase grid at/above) — but SKIPS the wasFirst loop (it defines the grid, offset stays the press anchor). Phrase-or-longer loops (L≥M) restart from block 0; sub-phrase use the phase formula.
- **CONSECUTIVE-LOOP UNIFIED QUANT GRID (505-like, load-bearing for sync):** every non-first loop locks to the first loop's start via ONE grid = powers-of-two of the phrase M, anchored to `masterPhase % M == 0` (phrase 0, since masterPhase is reset to 0 at first-loop define and only `++`/`%M` after). The four touchpoints share this grid: (1) latch starts the record (content) on the M/16 beat grid; (2) `_calcQuantizeTarget` quantizes LENGTH to the nearest of `{M/8,M/4,M/2,M,2M,4M,8M}`; (3) `_startEndingRecording` FLOORS `recordStartPhaseOffset` to the L-grid (`grid=min(L,M)`); (4) playback `play_block=(masterPhase-offset)%L`. **Content-start and phase-anchor are TIED:** `_startRecording` applies the SAME beat-snap delta to BOTH `m_recStartBlock` (audio) and `recordStartPhaseOffset` (phase), and `_startEndingRecording` subtracts the SAME `floorBack` from BOTH — so clip block 0's audio == the grid instant == phase zero. Independently snapping them was the "consecutive loops off by a quantized amount / fell apart after 2 loops" bug. Req1: the phrase M stays the RAW first-loop length (re-snapping M would shift the first loop off its exact region); `linkDeriveQuant` only sets the broadcast `s_bpm` to the nearest-120 multiple/division. Witnessed by `scripts/test-consec-lock.cpp` (`ALL PASS`: every L in {M/4..4M} phase-locks to phrase 0).
