...
- **Formant depth knob** (CC53): single-knob, ∈ [-1, +1] centred at data2=64 with ±4 deadzone.
  ...
  - **Pre-resample stage uses the SAME single-reader integer-period splice as the main pitch stage** (not a dual-reader frozen crossfade). At depth=0 the whole stage is bypassed (`m_formantDepth == 0.0f` → unity passthrough). Away from center `preRate ≠ 1` so the reader drifts vs the write pointer and must splice to stay in-bounds: a value+slope-matched WHOLE-number-of-periods jump (search `nn` in `periods±3`), trigger band (`gapLo=per*1.5`, `gapHi=per*8`) and crossfade length both scaled by `preRate` so neither reader moves >~1 period during the fade. **The earlier dual-reader frozen-crossfade clicked at the extremes**: at `depth=+1` (`preRate=2`) both readers kept advancing through a long frozen fade while the gap collapsed → re-wrap storm = the audible "doubling"; at `depth=-1` blind integer-period jumps clicked on real (consecutive-periods-differ) input. Host witness `scripts/test-formant-clicks.cpp` (runs depth −1/0/+1, counts click clusters): `depth=+1` 720→12, `depth=-1` 128→13, `depth=0`≈1. Whole-period displacement keeps the warp ratio exact, so the fix does not regress freq-neutrality (`scripts/test-freq-neutral.cpp` still PASS).
