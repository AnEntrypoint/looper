...
- **Formant depth knob** (CC53): single-knob, ∈ [-1, +1] centred at data2=64 with ±4 deadzone.
  ...
  - **Pre-resample stage uses a single-reader WSOLA best-correlation crossfaded splice** (material-agnostic). At depth=0 the whole stage is bypassed (`m_formantDepth == 0.0f` → unity passthrough — DEAD-CENTER MUST STAY BYTE-IDENTICAL, it is the proven-perfect path). Away from center `preRate = pow(scale,-depth) ≠ 1` so the single reader `m_preRd` drifts vs the write pointer and must splice to stay in-bounds. On `nearBound` it relocates the reader to the position within a ±`srch` window whose short grain (`GRAIN_N=24` samples at `preRate` stride) best CORRELATES (min SSD) with the grain currently being played, then ALWAYS crossfades from the pre-jump grain (`m_preRdB`) to the new position. This is material-agnostic: on a pitched tone the best match lands ~1 period away (displacement ≈ whole period = frequency-neutral, no detune); on drums/noise (SNAC can't lock, `per=0`) it still finds the most-similar splice point so transients survive and the jump is crossfaded.
  - **Evolution of this stage (do not regress):** (1) original dual-reader frozen-crossfade clicked at extremes — at `depth=+1` (`preRate=2`) both readers advanced through a long frozen fade while the gap collapsed → re-wrap storm = audible "doubling"; (2) single-reader integer-period match fixed the doubling/clicks on tones but on `depth<0` percussive material the unpitched `per=0` branch did a BARE RESET (raw discontinuity per gap-bound) = drums "completely scrambled", and `depth>0` stuttered from frequent backward period jumps; (3) current WSOLA best-match crossfaded splice fixes both — always crossfaded, never bare, correlation works with or without a period. Host witnesses: `scripts/test-formant-clicks.cpp` (tonal clicks: `depth=+1` 720→0, `depth=-1` 128→4), `scripts/test-formant-percussive.cpp` (drum onset preservation at all depths), `scripts/test-freq-neutral.cpp` (pitch still exact, 82→41.14 −0.15%).
