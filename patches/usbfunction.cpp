...
//
// LOOPER PATCH: tolerate a NULL current interface descriptor instead of
// asserting (which halts boot). A device whose configuration leaves the parser
// without a current interface descriptor -- e.g. a UAC2 interface (Tascam
// US-2x2) after our CUSBAudioDevice::Configure runs SelectInterfaceByClass for
// the UAC1 class triple and finds no match, which advances the parser past the
// last interface and nulls the current descriptor -- previously tripped
// `assert(m_pInterfaceDesc != 0)` in the copy constructor and HALTED the kernel
// at boot. Now both constructors tolerate the null and every accessor safe-fails
// (Configure returns FALSE, the class getters return 0xFF so nothing matches,
// GetNumEndpoints returns 0), so such a function is a harmless no-op the device
// layer skips -- enumeration of the rest of the bus continues and boot proceeds.
//
	...
	m_pInterfaceDesc (0)
{
	...
	// May be 0 for a degenerate/unsupported interface position -- tolerated.
	...
	m_pInterfaceDesc (0)
{
	...
	// May be 0 (see file header) -- tolerated, accessors safe-fail.
	m_pInterfaceDesc = (TUSBInterfaceDescriptor *) m_pConfigParser->GetCurrentDescriptor ();
}
...
boolean CUSBFunction::Configure (void)
{
	...
	{
		// No interface descriptor: nothing to configure, refuse cleanly so the
		// device layer skips this function (no halt).
		return FALSE;
	}
					      ...
					      unsigned nMinEndpoints)
{
	...
	// No current interface descriptor: no interface can match.
	if (m_pInterfaceDesc == 0)
	{
		return FALSE;
	}
...
{
	// May be 0 for a degenerate function (tolerated; caller must null-check).
