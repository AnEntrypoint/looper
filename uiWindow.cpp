...
static void serialMidiWrite4(unsigned char b0, unsigned char b1, unsigned char b2, unsigned char b3)
{
	if (!s_pSerial) return;
	s_pSerial->SetOptions(0);
	unsigned char buf[4] = { b0, b1, b2, b3 };
	s_pSerial->Write(buf, 4);
	s_pSerial->SetOptions(SERIAL_OPTION_ONLCR);
}

void sendSerialMidiCC(int cc_num, int value)
{
	serialMidiWrite4(0x0b, 0xb0, (unsigned char)cc_num, (unsigned char)value);
}

void sendSerialMidiNoteOn(u8 note, u8 velocity)
{
	serialMidiWrite4(0x09, 0x90, note, velocity);
}
...
void uiWindow::serialReceiveIRQHandler(unsigned char c)
{
	...
	else
	{
		...
		if (serial_midi_len == 4)
		{
			...
			else if ((serial_midi_buf[1] & 0xF0) == 0x90 ||	// Note On/Off from APC
					 (serial_midi_buf[1] & 0xF0) == 0x80)
			{
				if (pTheAPC)
					pTheAPC->handleMidi(serial_midi_buf[1], serial_midi_buf[2], serial_midi_buf[3]);
			}
			serial_midi_len = 0;
