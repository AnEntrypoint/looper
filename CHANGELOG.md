## 2026-07-08m — Live-tested ZCR against a real buzz occurrence: it's blind, add RMS-energy telemetry instead

- fix/feat: live-tested the ZCR telemetry (2026-07-08l, commit 607657a) on real hardware this session. Established a clean idle baseline via 20+ samples (izcL~3300-4300/sec, izcR~4500-5100/sec, inUR=0 throughout). Then sampled continuously while the user actively confirmed the buzz was audible: izcL dropped to 1900-2800/sec, izcR to 3400-4050/sec -- LOWER than idle, not higher, and inUR stayed 0 the entire window (zero underruns during the confirmed-audible buzz). Zero-crossing-rate is a HIGH-frequency-selective metric; this negative result means the buzz's energy likely sits at LOWER frequencies (mains hum, a beat tone, broadband noise skewed low) that barely register as zero-crossings, so ZCR was the wrong tool to have reached for first.
- Added a mean-absolute-amplitude accumulator (ienL/ienR fields, :4445 UAUD) at the SAME raw-input tap point (patches/usbaudiodevice.cpp InCompletion, before loopMachine/effects) that sums |sample| regardless of spectral shape -- unlike ZCR, this catches elevated noise energy of ANY frequency content, including low-frequency hum that a crossing-counter would miss. Exposed as a pre-divided 32-bit average (energy/sample-count, patches/kernel_run.cpp) to avoid both 64-bit CString::Format risk and 32-bit accumulator overflow over a multi-second window. Also confirmed via arm-none-eabi-nm/direct inspection this session: g_audioInPeak (the existing peak meter) is a MONOTONIC max latched since boot, not a live level -- useless for comparing idle-vs-buzz amplitude, which is exactly the gap ienL/ienR closes. Build-verified (kernel7l.img 1098276 bytes) and deployed to tftproot/7bec0617/kernel7l.img. Live re-test with the corrected metric is pending the user's next power-cycle.

## 2026-07-08l — Add raw-input zero-crossing-rate telemetry: user clarifies the buzz is filterable, not a click

- feat: user gave a critical correction to the prior session's theory -- engaging the HP/LP filter CLEANLY REMOVES the buzz entirely, not just colors/shapes it. This is the signature of continuous, band-limited tonal noise (like any steady tone or broadband hiss a filter attenuates), NOT an impulse/step discontinuity (a filter doesn't cleanly eliminate an impulse's ringing response the way it removes steady-state energy in a specific band). This directly invalidates the resync-crossfade fix (2026-07-08j, commit 14cbb95) as addressing the real mechanism -- that fix targeted smoothing a discrete jump, but the artifact described is continuous, not discrete.
- True audio capture (a WAV file via OTG to a host computer, or the existing continuousBuffer-based usbWavRecorder writing to a mounted USB drive) both require physical hardware unreachable from this dev-host session. Added the cheapest remotely-queryable alternative instead: zero-crossing-rate counters (izcL/izcR) on raw USB input, accumulated at the EARLIEST point in the pipeline (patches/usbaudiodevice.cpp CUSBAudioDevice::InCompletion, in the raw-decode loop, before loopMachine or any live-pitch/filter/effect touches the signal), exposed via the existing :4445 UAUD verb (patches/kernel_run.cpp), reset-on-read.
- At 48kHz, a dominant tone at frequency f produces roughly 2f zero-crossings/second -- sampling izcL/izcR once per second directly estimates the dominant frequency of any continuous artifact present at the USB wire itself, before the DSP chain ever touches it. This lets a live :4445 probe (no drive, no host computer, no cable swap) distinguish "the noise is genuinely on the raw input" from "the noise is introduced somewhere downstream in the engine" -- the single most diagnostic open question given the user's corrected symptom description. Build-verified (kernel7l.img 1097892 bytes) and deployed to tftproot/7bec0617/kernel7l.img; tftp-server.js running detached (started earlier this session) ready to serve on the Pi's next power-cycle.

## 2026-07-08k — Correction: this project uses XHCI, not DWHCI -- six prior fixes' driver attribution was wrong

- docs/fix: researched the actual vendored Circle driver source (both the local ~/.looper-build checkout and upstream rsta2/circle on GitHub) to verify the "GetResultLength() is broken for multi-packet iso URBs" claim underlying six-plus prior commits (2026-07-08c through j). The DWHCI mechanics cited (dwhcixferstagedata.cpp's TransactionComplete reassigning m_nTransferSize per-packet, GetResultLen() clamping to the last packet's size) are REAL and were verified line-by-line against the actual driver source -- but arm-none-eabi-nm on the ACTUAL BUILT kernel7l.elf shows CDWHCITransferStageData symbols entirely absent from the link; this RASPPI=4 build uses XHCI (the USB 3.0 host controller), not DWHCI (USB 2.0, older Pi models only). The entire prior root-cause narrative was a correct analysis of a driver file that has never been compiled into any deployed build on this hardware.
- Traced the REAL path this hardware uses: lib/usb/xhciendpoint.cpp CXHCIEndpoint::TransferEvent/EnqueueTRB. A multi-packet iso URB's N packets are independent TDs (no chain bit), with IOC (interrupt-on-completion) set only on the LAST packet's TRB -- so exactly one XHCI transfer event fires per URB, reporting the hardware's residual length for ONLY that final packet. SetResultLen(nBufLen - lastResidual) silently assumes every packet before the last delivered its full declared size, with no way to detect an early short one -- an OVER-report risk (claims more than was truly received), the OPPOSITE failure shape from DWHCI's under-report/truncation this project's history assumed.
- Net effect: NO functional/behavioral change was needed. patches/usbaudiodevice.cpp's existing m_nInSubmitBytes design (UAC2 InCompletion never calls GetResultLength() at all, trusting only what StartInRequest nominally submitted) already structurally avoids the XHCI over-report blind spot -- just not for the reason the code comments claimed. Considered patching xhciendpoint.cpp directly (setting IOC on every packet to get true per-packet actual-length tracking, like Linux's iso_frame_desc.actual_length) but rejected it: it would raise the USB completion-interrupt rate 8x on the hard-RT Core-0 ISR path (AGENTS.md: never block/overload USB completion handlers), an unverifiable-without-hardware regression risk for a bug class the existing design already avoids by a different mechanism.
- Fix: rewrote InCompletion's two comment blocks (patches/usbaudiodevice.cpp) to describe the actual XHCI mechanism instead of the inapplicable DWHCI one, so future sessions inherit correct driver attribution instead of re-deriving/re-verifying the same wrong-file analysis. Removed a same-session dead-code driver patch (patches/dwhcixferstagedata.cpp/.h, never committed) that would have compiled to nothing on this hardware. Build-verified (kernel7l.img byte-identical apart from comments) and deployed to tftproot/7bec0617/kernel7l.img.
- Why this matters going forward: six prior sessions' root-cause writeups (2026-07-08c through h) remain in CHANGELOG history as an accurate record of what was believed and done at the time -- not retroactively edited, per this project's own documentation policy (CHANGELOG carries history, present-tense source comments carry current truth). This entry is the correction layer; anyone reading those older entries for driver-level detail should read this one first.

## 2026-07-08j — Fix buzz shaped by filter: crossfade UAC2 IN ring resync jumps instead of stepping

- fix: user confirmed on real hardware (AIR 192|4, OTG not in use for this test) that the buzz is audible on the INPUT specifically, and that the live-effects FILTER (HP/LP/resonance) visibly shapes it -- the diagnostic signature of an impulsive/step discontinuity exciting a resonant IIR filter's ringing response, not a gradual rate-drift artifact (a filter doesn't distinctively color a slow bias). All six prior fixes (2026-07-08c through h) targeted the MAGNITUDE of a claimed-byte-count rate bias in patches/usbaudiodevice.cpp, never the SHAPE of the discontinuity at a resync event. Re-reading patches/input_usb.cpp's IN-ring resync (avail outside [AUDIO_BLOCK_SAMPLES, IN_RING_SIZE*3/4]) confirmed it hard-jumps the read position (rd = wr_snap - IN_TARGET_LAG, rd_frac=0) with zero continuity between the pre- and post-jump samples -- a genuine step every time it fires, exactly the kind of event a resonant filter downstream would audibly ring on. Fix: capture the last pre-jump sample and linearly crossfade it into the post-jump stream over IN_RESYNC_XFADE=16 samples, so the filter sees a ramp instead of a step -- without changing resync frequency or the underlying rate-tracking math from 2026-07-08h (kept as-is; this is a complementary fix to the jump's SHAPE, not a replacement for the MAGNITUDE work). Build-verified (kernel7l.img 1097572 bytes) and deployed to both tftproot/7bec0617/kernel7l.img and the physical SD card (E:/kernel7l.img). Live-hardware confirmation is pending the user's next test.

## 2026-07-08i — Add :4445 BUID verb: confirm the running kernel's build identity live

- feat: patches/kernel.cpp already computed build_id (a compile-date+time string) but only ever logged it once at boot via syslog. After the user reported the buzz+crunch cycle persisting UNCHANGED even after flashing the 2026-07-08h fix to both netboot and the physical SD card, a fresh diagnosis (fifth-dimension-engine route) identified the highest-prior explanation for "six locally-valid fixes, zero change in observed behavior" as a build/deploy verification gap rather than further micro-tuning of the same code path -- this project's own build system has documented repeat-offender stale-object classes (CHECK_DEPS=0 skipping header-triggered rebuilds, the `ar` re-add trap). Added a BUID verb to the existing :4445 UDP debug socket (patches/kernel_run.cpp) that reports build_id live, so a fix/flash/verify cycle can positively confirm the flashed binary actually booted without a syslog listener -- closing this class of ambiguity for every future cycle. Build-verified (kernel7l.img 1097444 bytes) and deployed to both tftproot/7bec0617/kernel7l.img and the physical SD card (E:/kernel7l.img). The underlying buzz+crunch cycle itself remains OPEN pending live :4445 DIAG/UAUD telemetry from the user's Pi to identify which counter's cadence actually matches the observed symptom (see fifth-dimension-engine route: the symptom could be IN resync, OUT resync, OTG tap resync, watchdog force-fire, or telemetry burst-drain -- never disambiguated by live measurement across any of the prior six fixes).

## 2026-07-08h — Fix buzz+crunch cycle persisting after 2026-07-08g: shrink the IN bias below the drift corrector's max absorb rate

- fix: patches/usbaudiodevice.cpp's 2026-07-08g bias (8134/8192, ~0.71% low) stopped the over-read but introduced a new problem: it made the claimed byte count a PERMANENT, GUARANTEED deficit on every microframe instead of an occasional one. input_usb.cpp's drift corrector (IN_TARGET_LAG=96, IN_DEADBAND=48, IN_RATE_MAX_DEV=64/IN_RATE_GAIN=131072) can only speed up the read rate by ~0.049% per step once outside the deadband, and only has IN_TARGET_LAG-AUDIO_BLOCK_SAMPLES=32 samples of slack before a hard resync -- a continuous 0.71% deficit (~14.5x the corrector's max absorb rate) drains that slack in ~0.1s of real time, forcing a resync on a fast, predictable cadence. This is why the user still reported buzzing AND the original underrun crunch+blips together on the same ~1-2s cycle even after 2026-07-08g: the fix eliminated the over-read but replaced it with an equally periodic controlled-deficit resync. Root-caused via node arithmetic this session confirming the bias/corrector-rate mismatch numerically. Fix: shrink the bias to 65500/65536 (~0.055% low), under the corrector's absorb ceiling and under typical USB clock tolerance (~0.25%), so the claim still clears the overwhelming majority of real microframes (avoiding over-read) while the residual deficit is small enough for the drift corrector to track continuously instead of resyncing. Build-verified (kernel7l.img 1097316 bytes) and deployed to tftproot/7bec0617/kernel7l.img (netboot path; no SD card mounted this session). Live-hardware confirmation of the audible fix is pending the user's next Pi session.
