...
function Make-Impulse([int]$sampleRate, [int]$totalSamples) {
    $buf = New-Object 'Int16[]' $totalSamples
    # Silence padding then a 2ms tone-burst pulse (96 samples @ 48kHz) at near-full scale.
    # Single-sample impulse is filtered out by anti-aliasing in the DAC chain.
    # 2ms Hann-windowed click is much more robust to detect AND still a well-defined onset.
    $impulseAt = [int]($sampleRate * 0.25)
    $pulseLen = 96
    for ($k = 0; $k -lt $pulseLen; $k++) {
        $w = 0.5 * (1.0 - [Math]::Cos(2.0 * [Math]::PI * $k / ($pulseLen - 1)))
        $buf[$impulseAt + $k] = [Int16]([Math]::Round(28000 * $w))
    }
    return ,$buf
}
...
function Find-OnsetSample([Int16[]]$capture, [int]$channels, [int]$thresholdAbs) {
    $stride = $channels
    ...
    # Threshold: 50% of observed peak OR 100 raw counts, whichever is smaller
    # (lower threshold than caller-provided so we catch attenuated signals)
    $thr = [Math]::Max(100, [int]($maxAbs * 0.5))
    for ($i = 0; $i -lt $n; $i++) {
        $v = [Math]::Abs([int]$capture[$i * $stride])
...
function Estimate-THD([Int16[]]$capture, [int]$channels, [double]$fund, [int]$sampleRate) {
...
function Run-Silence {
    ...
    $sil = New-Object 'Int16[]' ($SampleRate * 2)
